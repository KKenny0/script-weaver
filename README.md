...
未应用返回 422，`detail.code` 区分四种原因：`refine_no_meaningful_change`（无实质变化，仅备注/自动 ID/元数据变化不算）、`refine_constraint_failed`（受限修改越界，整体拒绝；普通修改新破坏引用完整性同样归入此码——`storyboard.shots[*].scene_id → script.scenes[*].scene_id`、`script.scenes[*].scene_design_id → scenes[*].id`、`visual_highlights[*].related_shot_ids → storyboard.shots[*].shot_id` 中新引入的悬空或歧义引用会被拒绝并指出字段，修改前快照已有的历史缺陷不阻断无关编辑）、`refine_not_executable`（路由决定不执行或目标不可修改，含 action/constraint 非字符串等非法类型）、`refine_target_not_found`（约束目标不存在，执行 Agent 不会被调用）。模型协议或请求失败返回脱敏的 502（`refine_model_failed`），不会伪装成未应用或成功。所有非 200 路径都不写库：revision、历史、memory 与产物保持原样。
