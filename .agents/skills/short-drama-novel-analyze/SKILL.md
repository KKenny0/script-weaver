...
当任务上下文来自 `script-weaver task context` 时，daemon 中的只读来源版本是唯一输入；不得读取任意本地原著路径，也不得按下文目录约定写文件。分析结果只通过 `script-weaver changeset submit` 提交为 typed `document.create` 或 `document.version.create` 候选。不得 apply ChangeSet、接受/退回文档版本或直接访问 SQLite。下文的本地目录、发布脚本和断点文件仅适用于独立安装模式。
