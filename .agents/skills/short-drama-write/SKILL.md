...
当任务上下文来自 `script-weaver task context` 时，daemon 中的只读来源版本与冻结 revisions 是唯一项目事实；不得按下文目录约定直接写剧本文件。写作结果只通过 `script-weaver changeset submit` 提交为 typed `document.create` 或 `document.version.create` 候选。不得 apply ChangeSet、接受/退回文档版本或直接访问 SQLite。下文的本地文件与脚本工作流仅适用于独立安装模式。

Script Weaver 只有在创作者接受时才做单向投影，并严格要求一级标题
`# EP001 集名`、连续场号与场景标题 `## EP001-SC001 内|外|内外 · 地点 · 时间`。
候选不符合该格式时保持待审批，不得声称已驱动分镜；修订必须提交新候选，不覆盖旧版本。
...
只有安装、升级或排障时运行 `python3 {技能目录}/scripts/selftest.py`。
