...
## Script Weaver 产品适配（优先）

当任务上下文来自 `script-weaver task context` 时，daemon 中的只读来源版本与冻结 revisions 是唯一项目事实；不得按下文目录约定直接写文件。开发结果只通过 `script-weaver changeset submit` 提交为 typed `document.create` 或 `document.version.create` 候选。不得 apply ChangeSet、接受/退回文档版本或直接访问 SQLite。下文的本地项目文件与发布生命周期仅适用于独立安装模式。

提交给 Script Weaver 的 development Markdown 必须在正文中恰好包含一个机器可读分集地图，
它与正文是同一个候选版本、一起审批，不另建文件或阶段：

````markdown
```script-weaver-episode-map
{"episode":1,"title":"第一集","story":"本集发生的主要行动、冲突升级与结尾状态变化。"}
{"episode":2,"title":"第二集","story":"承接上一集状态后，本集的新选择、代价与钩子。"}
```
````

每行的 `story` 必填且非空，必须概括本集真实行动、冲突与状态变化；它参与 daemon 的
分集 fingerprint，不能用标题复述或空泛占位。集号必须从 1 连续且唯一。多集整稿入口的每行还必须包含
`"source_span":{"start":0,"end":128,"sha256":"..."}`；start/end 是原始输入
UTF-8 bytes 的半开区间，按顺序、不重叠，sha256 对该 byte slice 计算。不得凭估算填写；
应复用 `episode_intake.py` 的精确索引结果。daemon 会在创作者接受时重新校验，任何一集失败
都不会创建部分 Episode。
