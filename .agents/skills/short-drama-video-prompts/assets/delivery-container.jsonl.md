...
`VID-13` 靠这条记录本地可证，不依赖阅读渲染文本。带 ✓ 的属于机械可证项，由 daemon 侧校验能力承接（见仓库 `src/script_weaver/capabilities/drama/`；本适配副本不含原仓库 `scripts/`），其余由审查者判定——此前这份清单整体写成「本地可证」，而脚本只实现了其中两项：
